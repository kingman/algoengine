...
public class BanzaiApplication implements Application {
       ...
       /**     LeavesQty leavesQty = new LeavesQty();
            ...
            **/
        	if(message.isSetField(LastQty.FIELD)){
	        	...
	        	fillSize = new BigDecimal(lastQty.getValue());
        	}
        }
    ...
    private void marketDataSnapshotFullRefresh(Message message, SessionID sessionID) throws FieldNotFound {
    	MDReqID mdReqID = (MDReqID) message.getField(new MDReqID());
    ...
    public void subscribe(String symbol, SessionID sessionID, String reqID){
    	...
    	if (beginString.equals(FixVersions.BEGINSTRING_FIX44)){
    		subscribe44(s, sessionID, reqID);
    	}
    }
    public void subscribe44(Symbol s, SessionID sessionID, String reqID){
    	...
    	marketDataRequest.addGroup(noMDEntryTypes);
    	noMDEntryTypes.set(new MDEntryType('3'));
    	marketDataRequest.addGroup(noMDEntryTypes);
    	
    	marketDataRequest.addGroup(noRelatedSym);
    	
    	send(marketDataRequest, sessionID);
    }
    ...
    public void replace44(Order order, Order newOrder) {
        quickfix.fix44.OrderCancelReplaceRequest message = new quickfix.fix44.OrderCancelReplaceRequest(
                new OrigClOrdID(order.getID()), new ClOrdID(newOrder.getID()), sideToFIXSide(order.getSide()), new TransactTime(),
                typeToFIXType(order.getType()));
